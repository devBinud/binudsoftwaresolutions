...
export default async function handler(req, res) {
  ...
  if (!process.env.SMTP_HOST || !process.env.SMTP_USER || !process.env.SMTP_PASS) {
    ...
    return res.status(500).json({
      ...
      details: `SMTP environment variables are not configured on Vercel: Missing ${missingVars.join(', ')}. Please add them to your Vercel Project Settings under Environment Variables.`
    });
  }
